...
![Initial decision page with Accept and Reject radio buttons](/tasklist/check-new-pcd-case-iteration/01-decision.png)

### Accept flow - RASSO cases

If the user chooses 'Accept', they're asked what type of review task this is.

![Review task type page with three options: Early advice, Within 5 calendar days, Within 28 calendar days](/tasklist/check-new-pcd-case-iteration/02-accept-review-task-type.png)

When they select 'Early advice', they must select case type.

![Case type page with options: Magistrate court, Crown court, RASSO, Complex Casework Unit](/tasklist/check-new-pcd-case-iteration/03-accept-case-type.png)

If they select 'RASSO', they're asked if they want to transfer the case. The page shows the current unit in inset text.

![Transfer case page asking 'Do you want to transfer the case?' with Yes/No options and current unit shown in inset](/tasklist/check-new-pcd-case-iteration/04-accept-rasso-transfer-case.png)
...
![Task owner page with autocomplete field asking who to assign the early advice manager triage task to](/tasklist/check-new-pcd-case-iteration/05-accept-rasso-task-owner.png)

The autocomplete shows both individuals with their role in brackets and teams. When they type, matching results are shown.

![Autocomplete showing filtered results including users and teams](/tasklist/check-new-pcd-case-iteration/05a-accept-rasso-task-owner-autocomplete.png)

After selecting a task owner, they see the check answers page.

![Check answers page showing: Decision (Accept), Review task type (Early advice), Case type (RASSO), Transfer case (No), Task owner selection](/tasklist/check-new-pcd-case-iteration/06-accept-rasso-check-answers.png)

### Accept flow - non-RASSO cases

If they select a case type other than RASSO, they still see the transfer case question.

![Transfer case page for non-RASSO case](/tasklist/check-new-pcd-case-iteration/07-accept-non-rasso-transfer-case.png)

If they choose not to transfer, they go to the page to select a prosecutor to assign to the case.

![Prosecutor page with autocomplete field](/tasklist/check-new-pcd-case-iteration/08-accept-non-rasso-prosecutor.png)

The autocomplete filters the list of prosecutors as they type.

![Prosecutor autocomplete showing filtered results](/tasklist/check-new-pcd-case-iteration/08a-accept-non-rasso-prosecutor-autocomplete.png)

The check answers page shows the prosecutor assignment instead of a task owner.

![Check answers page for non-RASSO showing prosecutor assignment](/tasklist/check-new-pcd-case-iteration/09-accept-non-rasso-check-answers.png)

### Transferring the case to another unit

When users choose to transfer the case, they're asked if they want to change the area. The current area is shown in inset text.

![Area page asking if they want to change the area, with current area shown](/tasklist/check-new-pcd-case-iteration/10-accept-transfer-area.png)

If they select 'Yes', an autocomplete field is revealed where they can select a different area.

![Area autocomplete conditionally revealed after selecting Yes](/tasklist/check-new-pcd-case-iteration/10a-accept-transfer-area-revealed.png)

The autocomplete shows matching areas as they type.

![Area autocomplete showing filtered results](/tasklist/check-new-pcd-case-iteration/10b-accept-transfer-area-autocomplete.png)

Next, they select which unit to transfer the case to. The current unit is shown in inset text.

![Unit page with autocomplete, showing current unit](/tasklist/check-new-pcd-case-iteration/11-accept-transfer-unit.png)

The unit autocomplete is filtered by the selected area.

![Unit autocomplete showing filtered results](/tasklist/check-new-pcd-case-iteration/11a-accept-transfer-unit-autocomplete.png)
...
![Check answers page showing full transfer details](/tasklist/check-new-pcd-case-iteration/12-accept-transfer-check-answers.png)
...
If the user chooses 'Reject' at the start, they're asked to select reasons for rejection from a list of checkboxes.

![Reasons for rejection page with multiple checkbox options](/tasklist/check-new-pcd-case-iteration/13-reject-reasons.png)

Selecting a checkbox, conditionally reveals a character count to add details.

![Textarea revealed after selecting 'No MG3' checkbox](/tasklist/check-new-pcd-case-iteration/13a-reject-reasons-revealed.png)

The user can select multiple reasons.

![Multiple checkboxes selected with revealed textareas](/tasklist/check-new-pcd-case-iteration/13b-reject-reasons-multiple.png)

After selecting reasons, they're asked what type of review task this is.

![Review task type page in reject flow](/tasklist/check-new-pcd-case-iteration/14-reject-review-task-type.png)

Then they're asked to identify the case type.

![Case type page in reject flow](/tasklist/check-new-pcd-case-iteration/15-reject-case-type.png)

When rejecting a case, the system sends a message to the police. The user must provide the date the police should respond by.

![Police response date page with day, month, year inputs](/tasklist/check-new-pcd-case-iteration/16-reject-police-response-date.png)

The user is then asked if they want to create a reminder task to follow up when the police should respond.

![Create reminder task page with Yes/No radio buttons](/tasklist/check-new-pcd-case-iteration/17-reject-create-reminder-task.png)

If they select 'Yes', a date input is conditionally revealed where they can set the reminder date.

![Reminder date input revealed after selecting Yes](/tasklist/check-new-pcd-case-iteration/17a-reject-reminder-task-revealed.png)

The check answers page for a rejection shows all the rejection details.

![Check answers page for reject flow showing all details](/tasklist/check-new-pcd-case-iteration/18-reject-check-answers.png)

### Completing the task

When they click 'Complete task', they're redirected to the case task list with a success banner.

![Success banner showing 'Task completed'](/tasklist/check-new-pcd-case-iteration/19-success-message.png)
